...
func _physics_process(_delta):
	...
	wiggle += _delta
	scale = Vector2.ONE * max(0.8, (sin(wiggle)*wiggle_amount) * 0.35)
...
func die():
	...
	if Effects != null:
		...
		$Tween.start()
		$Tween.interpolate_property(self, "scale", scale, Vector2(0,0), scale_time, Tween.TRANS_BACK, Tween.EASE_IN)
